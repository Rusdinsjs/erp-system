...
impl ContractRenewalRepository {
    ...
    pub async fn create(&self, renewal: &ContractRenewal) -> Result<ContractRenewal, DomainError> {
        let result = sqlx::query_as::<_, ContractRenewal>(
            r#"
    ...
    pub async fn get_by_id(&self, id: Uuid) -> Result<ContractRenewal, DomainError> {
        let renewal =
            sqlx::query_as::<_, ContractRenewal>("SELECT * FROM contract_renewals WHERE id = $1")
                .bind(id)
                .fetch_one(&self.pool)
                .await
                .map_err(|e| match e {
    ...
    pub async fn list_by_contract(
        ...
        let renewals = sqlx::query_as::<_, ContractRenewal>(
            r#"
