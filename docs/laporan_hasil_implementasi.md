...
### IV. PEMBATASAN AKSES MASTER DATA DAN SESI AUDIT

Untuk memperketat keamanan dan sesuai dengan rekomendasi sistem, hak akses pengelolaan master data (Kategori, Template Kategori, Lokasi) serta pelaksanaan Sesi Audit dibatasi berdasarkan level tingkatan pengguna (`role_level`):

1. **Kategori & Template Kategori (Level L2 - Manager ke atas)**:
   * Pembuatan, pembaruan, dan penghapusan (`create`, `update`, `delete`) dibatasi secara ketat hanya untuk **Super Admin (L1)** dan **Manager (L2)** (`role_level <= 2`).
   * Admin Spesialis (L4) hanya memiliki hak akses membaca (Read-Only).

2. **Lokasi (Level L2 - Manager ke atas)**:
...
### V. VERIFIKASI DAN KUALITAS KODE
