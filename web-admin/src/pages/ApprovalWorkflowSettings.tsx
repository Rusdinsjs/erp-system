        ...
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-slate-400 mt-1">Configure multi-level approval steps for documents</p>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {workflows.map((workflow) => (
                    <Card key={workflow.id} padding="lg" className="hover:border-slate-700 transition-colors">
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex-1">
                                <h3 className="text-lg font-semibold text-white">{workflow.workflow_name}</h3>
                                <p className="text-sm text-indigo-400 font-medium uppercase tracking-wider mt-1">
                                    {workflow.entity_type}
                                </p>
                            </div>
                            <span className={`px-2 py-1 text-xs rounded-full ${workflow.is_active ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-500/10 text-slate-500'}`}>
                        ...
                        <div className="space-y-3 mb-6">
                            <div className="flex items-center justify-between text-sm">
                                ...
                                <span className="text-white font-medium">{workflow.approval_levels} Steps</span>
                            </div>
                            <div className="border-t border-slate-800 pt-3">
                                <p className="text-xs text-slate-500 uppercase font-bold mb-2">Approval Chain</p>
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-slate-800 text-white rounded hover:bg-slate-700 transition-colors"
                            >
                                <Edit2 size={16} /> Edit
                            </button>
                            <button
                                onClick={() => handleDelete(workflow.id)}
                                className="px-3 py-2 bg-red-500/10 text-red-500 rounded hover:bg-red-500 hover:text-white transition-all"
                            >
                                <Trash2 size={16} />
                            </button>
                        </div>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-2xl max-w-xl w-full overflow-hidden">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Workflow Name</label>
                                    <input
                                        type="text"
                                        required
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Approval Levels (1-5)</label>
                                <input
                                    type="number"
                                    min="1"
                                    max="5"
                            ...
                            <div className="space-y-4 border-t border-slate-800 pt-4">
                                <p className="text-xs font-bold text-slate-500 uppercase">Role Assignments</p>
                                {[1, 2, 3, 4, 5].slice(0, formData.approval_levels).map(lvl => (
                                    <div key={lvl} className="flex items-center gap-4">
                                        <span className="text-sm text-slate-400 w-16">Level {lvl}:</span>
                                        <select
                                            value={(formData as any)[`level_${lvl}_role`]}
                                            onChange={(e) => setFormData({ ...formData, [`level_${lvl}_role`]: e.target.value })}
                                            className="flex-1 bg-slate-950 border border-slate-800 rounded-lg px-4 py-2 text-white text-sm outline-none"
                                        >
                            ...
                            <div className="flex items-center gap-3 pt-4">
                                ...
                                />
                                <label htmlFor="is_active" className="text-sm font-medium text-slate-300">Set as Active Workflow</label>
                            </div>
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <button
                                    type="submit"
                                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition shadow-lg shadow-indigo-600/20"
                                >
