    ...
    return (
        <div className="p-8">
            ...
            <div className="flex justify-between items-end mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight">Stock Opname</h1>
                    <p className="text-gray-400 mt-2">Reconcile physical inventory with system records</p>
                </div>
                <div className="flex gap-3">
                    ...
                    <Button
                        ...
                        className="rounded-xl shadow-lg shadow-emerald-500/20 bg-emerald-600 hover:bg-emerald-700 text-white border-none"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <Card className="relative overflow-hidden group p-6 border-white/5 bg-gray-800/50">
                    ...
                    <div className="flex justify-between items-start relative z-10">
                        <div>
                            <p className="text-gray-400 text-sm font-medium">Items Counted</p>
                            <h3 className="text-3xl font-bold text-white mt-1">{itemsCounted}</h3>
                        </div>
                ...
                <Card className="relative overflow-hidden group p-6 border-white/5 bg-gray-800/50">
                    <div className={`absolute top-0 right-0 w-24 h-24 rounded-full -mr-4 -mt-4 transition-transform group-hover:scale-110 ${itemsWithDiscrepancy > 0 ? 'bg-amber-500/10' : 'bg-gray-500/10'}`} />
                    <div className="flex justify-between items-start relative z-10">
                        <div>
                            <p className="text-gray-400 text-sm font-medium">Discrepancies</p>
                            <h3 className={`text-3xl font-bold mt-1 ${itemsWithDiscrepancy > 0 ? 'text-amber-400' : 'text-white'}`}>
                                {itemsWithDiscrepancy}
                            </h3>
                        </div>
                        <div className={`p-3 rounded-xl ${itemsWithDiscrepancy > 0 ? 'bg-amber-500/20' : 'bg-gray-500/20'}`}>
                            <AlertCircle className={itemsWithDiscrepancy > 0 ? 'text-amber-400' : 'text-gray-400'} size={24} />
                        </div>
                    </div>
                </Card>
            </div>
            ...
            <Card className="overflow-hidden p-0 border-white/5 bg-gray-800/40 backdrop-blur-sm">
                {/* Search & Filters Bar */}
                <div className="p-4 border-b border-white/5 flex gap-4 bg-gray-900/30">
                    <div className="flex-1 max-w-xl">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2.5 bg-gray-900/50 border border-white/10 rounded-xl text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
                                value={searchTerm}
                    ...
                    <div className="flex-1">
                        <input
                            ...
                            className="w-full px-4 py-2.5 bg-gray-900/50 border border-white/10 rounded-xl text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
                            value={globalNote}
                        ...
                        <Table className="border-none">
                            ...
                            <TableBody>
                                {filteredItems.map((item: InventoryItem) => {
                                    ...
                                    return (
                                        <TableRow key={item.id} className={`border-white/5 transition-colors ${hasChange ? 'bg-amber-500/5 hover:bg-amber-500/10' : 'hover:bg-gray-700/30'}`}>
                                            <TableTd>
                                            ...
                                            <TableTd align="right">
                                                <input
                                                    type="number"
                                                    className={`w-32 px-3 py-1.5 text-right bg-gray-900 border rounded-lg focus:ring-2 outline-none transition-all ${hasChange
                                                        ? 'border-amber-500/50 focus:ring-amber-500/50 text-amber-100'
                                                        : 'border-white/10 focus:ring-blue-500/50 text-gray-200 focus:border-blue-500/50'
                                                        }`}
