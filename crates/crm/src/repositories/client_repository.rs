...
impl ClientRepository {
    ...
    pub async fn create(&self, client: &Client) -> Result<Client, sqlx::Error> {
        sqlx::query_as!(
            Client,
            r#"
            INSERT INTO crm.clients (id, client_code, name, company_name, email, phone, address, city, contact_person, tax_id, is_active, notes, created_at, updated_at)
    ...
    pub async fn find_by_id(&self, id: Uuid) -> Result<Option<Client>, sqlx::Error> {
        sqlx::query_as!(
            Client,
            r#"SELECT id, client_code, name, company_name, email, phone, address, city, contact_person, tax_id, is_active, notes, created_at, updated_at FROM crm.clients WHERE id = $1"#,
            id
        )
    ...
    pub async fn find_by_code(&self, code: &str) -> Result<Option<Client>, sqlx::Error> {
        sqlx::query_as!(
            Client,
            r#"SELECT id, client_code, name, company_name, email, phone, address, city, contact_person, tax_id, is_active, notes, created_at, updated_at FROM crm.clients WHERE client_code = $1"#,
            code
        )
        .fetch_optional(&self.pool)
        .await
    ...
    pub async fn count(&self) -> Result<i64, sqlx::Error> {
        let result = sqlx::query_scalar!(r#"SELECT COUNT(*) as "count!" FROM crm.clients"#)
            .fetch_one(&self.pool)
            .await?;
        Ok(result)
    }
    ...
    pub async fn delete(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM crm.clients WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn soft_delete(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result =
            sqlx::query("UPDATE crm.clients SET is_active = false, updated_at = NOW() WHERE id = $1")
                .bind(id)
                .execute(&self.pool)
                .await?;
        Ok(result.rows_affected() > 0)
    ...
    /// Force delete a client by deleting dependent records first
