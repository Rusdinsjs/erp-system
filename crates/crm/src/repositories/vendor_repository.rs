...
impl VendorRepository {
    ...
    pub async fn find_by_id(&self, id: Uuid) -> Result<Option<Vendor>, sqlx::Error> {
        sqlx::query_as::<_, Vendor>("SELECT * FROM crm.vendors WHERE id = $1")
            .bind(id)
    ...
    pub async fn find_by_name(&self, name: &str) -> Result<Option<Vendor>, sqlx::Error> {
        sqlx::query_as::<_, Vendor>("SELECT * FROM crm.vendors WHERE name = $1")
            .bind(name)
            .fetch_optional(&self.pool)
            .await
    }
    ...
    pub async fn create(&self, vendor: &Vendor) -> Result<Vendor, sqlx::Error> {
        sqlx::query_as::<_, Vendor>(
            r#"
            INSERT INTO crm.vendors (id, code, name, contact_person, phone, email, address, is_active)
    ...
    pub async fn delete(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let result = sqlx::query("DELETE FROM crm.vendors WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await?;
