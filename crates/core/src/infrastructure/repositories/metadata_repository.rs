...
impl MetadataRepository {
    ...
    pub async fn create_entity_type(&self, entity_type: &EntityType) -> Result<EntityType, sqlx::Error> {
        let rec = sqlx::query_as!(
            ...
            RETURNING id, name, module, storage_strategy as "storage_strategy: StorageStrategy", is_custom, version, created_at, updated_at
            "#,
            entity_type.id,
            entity_type.name,
            entity_type.module,
            entity_type.storage_strategy.clone() as StorageStrategy,
    ...
    pub async fn find_entity_type_by_name(&self, name: &str) -> Result<Option<EntityType>, sqlx::Error> {
        let rec = sqlx::query_as!(
            EntityType,
            r#"
            SELECT id, name, module, storage_strategy as "storage_strategy: StorageStrategy", is_custom, version, created_at, updated_at
            FROM entity_types
            WHERE name = $1
            "#,
            name
