...
impl ContractApprovalRepository {
    ...
    pub async fn create(
        ...
        let result = sqlx::query_as::<sqlx::Postgres, ContractApproval>(
            r#"INSERT INTO contract_approvals (id, contract_id, approver_id, action, notes, approval_level, delegated_to, created_at)
    ...
    pub async fn find_by_contract(
        ...
        let approvals = sqlx::query_as::<sqlx::Postgres, ContractApproval>(
            r#"SELECT id, contract_id, approver_id, action, notes, approval_level, delegated_to, created_at
    ...
    pub async fn find_by_contract_with_names(
        &self,
        contract_id: Uuid,
    ) -> Result<Vec<management_system_core::application::dto::contract_dto::ApprovalResponse>, DomainError> {
        let rows = sqlx::query(
            r#"SELECT
                ca.id,
                ca.contract_id,
        ...
        Ok(rows
            .into_iter()
            .map(
                |row| management_system_core::application::dto::contract_dto::ApprovalResponse {
                    id: row.get("id"),
                    ...
                    delegated_to_name: row.get("delegated_to_name"),
    ...
    pub async fn find_latest(
        ...
        let approval = sqlx::query_as::<sqlx::Postgres, ContractApproval>(
            r#"SELECT id, contract_id, approver_id, action, notes, approval_level, delegated_to, created_at
            FROM contract_approvals
            WHERE contract_id = $1
            ORDER BY created_at DESC
    ...
    pub async fn count_pending(&self) -> Result<i64, DomainError> {
        let row = sqlx::query(
            r#"SELECT COUNT(DISTINCT ca.contract_id) as count
            FROM contract_approvals ca
