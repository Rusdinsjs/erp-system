...
impl ContractRepository {
    ...
    pub async fn create(&self, contract: &RentalContract) -> DomainResult<RentalContract> {
        let rec = sqlx::query_as::<sqlx::Postgres, RentalContract>(
            r#"INSERT INTO rental_contracts (
    ...
    pub async fn find_by_client(&self, client_id: Uuid) -> DomainResult<Vec<RentalContract>> {
        let recs = sqlx::query_as::<sqlx::Postgres, RentalContract>(
            r#"SELECT
                id, contract_number, client_id, NULL::text as client_name, start_date, end_date,
                auto_renew, renewal_notice_days, payment_terms, price_lock,
                status, contract_file_url, notes,
    ...
    pub async fn find_expiring_soon(&self, days: i32) -> DomainResult<Vec<RentalContract>> {
        let recs = sqlx::query_as::<sqlx::Postgres, RentalContract>(
            r#"SELECT
                ...
                rc.current_approval_step,
                rc.total_approval_steps,
                rc.template_id,
                rc.delegated_to
            FROM rental_contracts rc
            LEFT JOIN clients c ON rc.client_id = c.id
    ...
    pub async fn get_contract_usage(
        ...
        let total_sheets: i64 = row.get::<Option<i64>, _>("total_sheets").unwrap_or(0);
        let total_op: rust_decimal::Decimal = row.get::<Option<rust_decimal::Decimal>, _>("total_operating").unwrap_or_default();
        let total_sb: rust_decimal::Decimal = row.get::<Option<rust_decimal::Decimal>, _>("total_standby").unwrap_or_default();
        let total_bd: rust_decimal::Decimal = row.get::<Option<rust_decimal::Decimal>, _>("total_breakdown").unwrap_or_default();

        Ok((
            total_sheets,
            total_op.to_f64().unwrap_or(0.0),
            total_sb.to_f64().unwrap_or(0.0),
            total_bd.to_f64().unwrap_or(0.0),
        ))
    }
