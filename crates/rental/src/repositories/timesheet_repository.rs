...
use management_system_core::application::dto::TimesheetDetailResponse;
...
impl TimesheetRepository {
    ...
    pub async fn create_timesheet(
        ...
        .bind(timesheet.id).bind(timesheet.rental_id).bind(timesheet.rental_item_id)
        .bind(timesheet.work_date).bind(timesheet.start_time).bind(timesheet.end_time)
        .bind(timesheet.operating_hours).bind(timesheet.standby_hours)
        .bind(timesheet.overtime_hours).bind(timesheet.breakdown_hours)
        .bind(timesheet.hm_km_start).bind(timesheet.hm_km_end).bind(timesheet.hm_km_usage)
        .bind(&timesheet.operation_status).bind(&timesheet.breakdown_reason)
        .bind(&timesheet.work_description).bind(&timesheet.work_location)
        .bind(&timesheet.photos).bind(timesheet.checker_id).bind(timesheet.checker_at)
        .bind(&timesheet.checker_notes).bind(&timesheet.status)
        .bind(timesheet.production_volume).bind(&timesheet.production_unit)
        .bind(timesheet.fuel_consumed_liters)
        .fetch_one(&self.pool)
        .await
    }
    ...
    pub async fn update_timesheet(
        ...
        .bind(timesheet.id).bind(timesheet.start_time).bind(timesheet.end_time)
        .bind(timesheet.operating_hours).bind(timesheet.standby_hours)
        .bind(timesheet.overtime_hours).bind(timesheet.breakdown_hours)
        .bind(timesheet.hm_km_start).bind(timesheet.hm_km_end).bind(timesheet.hm_km_usage)
        .bind(&timesheet.operation_status).bind(&timesheet.breakdown_reason)
        .bind(&timesheet.work_description).bind(&timesheet.work_location)
        .bind(&timesheet.photos).bind(&timesheet.checker_notes)
        .bind(timesheet.production_volume).bind(&timesheet.production_unit)
        .bind(timesheet.fuel_consumed_liters).bind(now)
        .fetch_one(&self.pool)
        .await
    }
    ...
    pub async fn sum_timesheets_for_period(
        ...
        sqlx::query_as::<sqlx::Postgres, TimesheetSummaryRow>(
            r#"SELECT
    ...
    pub async fn create_billing_period(
        ...
        .bind(billing.id).bind(billing.rental_id)
        .bind(billing.period_start).bind(billing.period_end)
        .bind(&billing.period_type).bind(&billing.status)
        .bind(billing.total_production_volume).bind(billing.total_fuel_consumed)
        .bind(billing.fuel_surcharge_rate).bind(billing.fuel_surcharge_amount)
        .fetch_one(&self.pool)
        .await
    }
    ...
    pub async fn create_client_contact(
        ...
        sqlx::query_as::<sqlx::Postgres, ClientContact>(
        ...
        .bind(contact.id).bind(contact.client_id)
        .bind(&contact.name).bind(&contact.position)
        .bind(&contact.email).bind(&contact.phone)
        .bind(contact.can_approve_timesheet).bind(contact.can_approve_billing)
        .bind(contact.approval_limit).bind(contact.is_primary).bind(contact.is_active)
        .fetch_one(&self.pool)
        .await
    }
