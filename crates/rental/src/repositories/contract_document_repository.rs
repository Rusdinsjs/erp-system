...
impl ContractDocumentRepository {
    ...
    pub async fn create(&self, document: &ContractDocument) -> DomainResult<ContractDocument> {
        let rec = sqlx::query_as::<sqlx::Postgres, ContractDocument>(
            r#"INSERT INTO contract_documents (
    ...
    pub async fn find_by_id(&self, id: Uuid) -> DomainResult<Option<ContractDocument>> {
        let rec = sqlx::query_as::<sqlx::Postgres, ContractDocument>(
            r#"SELECT * FROM contract_documents WHERE id = $1"#,
        )
        .bind(id)
    ...
    pub async fn find_by_contract_id(
        ...
        let recs = sqlx::query_as::<sqlx::Postgres, ContractDocument>(
            r#"SELECT * FROM contract_documents
            WHERE contract_id = $1
    ...
    pub async fn get_next_version(
        ...
        Ok(row.get::<Option<i32>, _>("next_version").unwrap_or(1))
    }
    ...
    pub async fn delete(&self, id: Uuid) -> DomainResult<()> {
        sqlx::query("DELETE FROM contract_documents WHERE id = $1")
            .bind(id)
            .execute(&self.pool)
            .await
            .map_err(|e| DomainError::Database(e.to_string()))?;
    ...
    pub async fn count_by_contract(&self, contract_id: Uuid) -> DomainResult<i64> {
        let row = sqlx::query(
            r#"SELECT COUNT(*) as count FROM contract_documents
            WHERE contract_id = $1 AND is_active = true"#,
        )
        .bind(contract_id)
        .fetch_one(&self.pool)
