...
impl BillingService {
    ...
    pub async fn calculate_billing(
        ...
        let rate_id = if let Some(item_id) = billing.rental_item_id {
            ...
            if let Some(item) = item_opt {
                use sqlx::Row;
                item.get::<Option<Uuid>, _>("rental_rate_id")
            } else {
                None
            }
        } else {
        ...
        if let Some(ref rate) = rate {
            ...
            let is_fuel_included = if let Some(item_id) = billing.rental_item_id {
                ...
                if let Some(row) = item_res {
                    use sqlx::Row;
                    row.get::<Option<bool>, _>("is_fuel_included").unwrap_or(false)
                } else {
                    false
                }
            } else {
    ...
    pub async fn get_billing_summary(
        &self,
        billing_id: Uuid,
    ) -> DomainResult<BillingSummaryResponse<crate::domain::entities::RentalTimesheet>> {
        let billing = self.get_by_id(billing_id).await?;
        ...
        Ok(BillingSummaryResponse {
            ...
            rental_number: {
                use sqlx::Row;
                rental_info.get::<String, _>("rental_number")
            },
            client_name: {
                use sqlx::Row;
                rental_info.get::<String, _>("client_name")
            },
            asset_name: {
                use sqlx::Row;
                rental_info.get::<String, _>("asset_name")
            },
