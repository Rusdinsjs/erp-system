...
impl InventoryService {
    ...
    pub async fn adjust_stock_batch(
        &self,
        req: management_system_core::application::dto::BatchInventoryAdjustmentRequest,
        user_id: Option<Uuid>,
    ) -> DomainResult<bool> {
        // Ideally this should be a transaction, but for now we iterate
        for item in req.items {
            let adjust_req = management_system_core::application::dto::InventoryAdjustmentRequest {
