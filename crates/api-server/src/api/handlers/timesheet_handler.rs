...
pub async fn update_timesheet(
    ...
    Extension(claims): Extension<UserClaims>,
    Path(id): Path<Uuid>,
    Json(request): Json<management_system_core::application::dto::UpdateTimesheetRequest>,
...
) -> AppResult<Json<management_system_core::application::dto::TimesheetSummary>> {
    let summary = state
        .timesheet_service
...
pub async fn list_pending_verification(
    State(state): State<AppState>,
) -> AppResult<Json<Vec<management_system_core::application::dto::TimesheetDetailResponse>>> {
